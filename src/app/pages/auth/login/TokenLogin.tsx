  ...
  return (
    ...
    >
      ...
      <Box direction="Column" gap="100">
        <Text size="L400">{t('auth.login.token_login.title')}</Text>
        <Text size="T300">
          <b>{message}</b>
        </Text>
      </Box>
      ...
      {loginState.status === AsyncStatus.Error && (
        <>
          {loginState.error.errcode === LoginError.Forbidden && (
            <LoginTokenError message={t('auth.login.token_login.invalid_login_token')} />
          )}
          {loginState.error.errcode === LoginError.UserDeactivated && (
            <LoginTokenError message={t('auth.login.token_login.account_has_been_deactivated')} />
          )}
          {loginState.error.errcode === LoginError.InvalidRequest && (
            <LoginTokenError message={t('auth.login.token_login.failed_to_login_request_data_is_invalid')} />
          )}
          {loginState.error.errcode === LoginError.RateLimited && (
            <LoginTokenError message={t('auth.login.token_login.failed_to_login_request_has_been_rate_limited')} />
          )}
          {loginState.error.errcode === LoginError.Unknown && (
            <LoginTokenError message={t('auth.login.token_login.failed_to_login_unknown_reason')} />
          )}
        </>
      )}
